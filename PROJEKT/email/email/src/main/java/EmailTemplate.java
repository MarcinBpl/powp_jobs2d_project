...
public class EmailTemplate extends Application {
	...
	//Observer list
	public static final ObservableList<Template> TEMPLATES = FXCollections.observableArrayList();
			...
			public void handle(ActionEvent e) {
				if (templateName.getText().length() > 0) {
					if (!TEMPLATES.contains(findObject(templateName.getText(), TEMPLATES))) {
						...
						TEMPLATES.add(TemplateBuilder.aTemplate()
								.withName(templateName.getText())
								.withValue(htmlEditor.getHtmlText()).build());
